// Servicio para obtener negocios del usuario
...
function normalizeBusinessDTO(dto: BusinessDTO): Business {
  return {
    id: String(dto.externalBusinessId), // Usamos el ID externo (del restaurante/hospedaje real)
    name: dto.name,
    type: dto.businessType,
    address: '', // Se puede obtener después si es necesario
    imageUrl: undefined,
    email: undefined,
    phone: undefined
  };
...
export async function fetchUserBusinesses(userId: string): Promise<Business[]> {
  try {
    // Endpoint correcto del backend: /business/user/{userId}
