...
import {
  Card, Row, Col, Button, Badge, Form, Alert, Spinner, Tabs, Tab
} from 'react-bootstrap';
import { fetchPedidosByRestaurant, updateEstadoPedido } from '../../../../services/fetchPedidos';
import { useWebSocket } from '../../hooks/useWebSocket';
import type { Pedido, EstadoPedido } from '../../types';
import { getEstadoPedidoBadge, getSiguienteEstadoPedido } from '../../types';

interface GastronomiaAdministrarProps {
  businessId: string;
  businessName: string;
}

export function GastronomiaAdministrar({ businessId, businessName }: GastronomiaAdministrarProps) {
  ...
  const pedidosActivos = pedidosFiltrados.filter(p =>
    p.status !== 'DELIVERED' as EstadoPedido
  );
  ...
  const handleCambiarEstado = async (pedido: Pedido) => {
    ...
    try {
      ...
      if (result) {
        ...
        setMensaje({ tipo: 'success', texto: `Pedido marcado como ${getEstadoPedidoBadge(siguienteEstado).texto}` });
      }
    } catch (error) {
  ...
  const calcularTotales = () => {
    ...
    return {
      ...
      efectivo: filtrados.filter(p => p.paidWithCash).reduce((sum, p) => sum + (p.totalPrice || p.totalAmount || 0), 0),
      mercadoPago: filtrados.filter(p => p.paidWithMercadoPago).reduce((sum, p) => sum + (p.totalPrice || p.totalAmount || 0), 0)
    };
  };
  ...
  return (
    <div>
      ...
      <Tabs activeKey={activeTab} onSelect={(k) => setActiveTab(k as 'activos' | 'historial')} className="mb-3">
        {/* Tab Pedidos Activos */}
    ...
    <Card className="h-100">
      ...
      <Card.Body>
        ...
        <div className="mb-2">
          ...
          {pedido.paidWithCash && (
            <Badge bg="secondary">💵 Efectivo</Badge>
          )}
      ...
      {!showHistorial && siguienteEstado && (
        <Card.Footer>
          <Button
            variant={estadoBadge.color === 'secondary' ? 'primary' : estadoBadge.color === 'info' ? 'success' : 'dark'}
            className="w-100"
