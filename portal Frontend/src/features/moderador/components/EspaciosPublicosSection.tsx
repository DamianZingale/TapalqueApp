...
export function EspaciosPublicosSection() {
    ...
    const handleSelect = (item: EspacioPublico) => { setSelected(item); setIsNew(false); setFormData({ ...item }); };
    ...
    const handleDelete = async () => {
        ...
        try {
            const token = localStorage.getItem("token");
            const res = await fetch(`/api/espacio-publico/${selected.id}`, { method: "DELETE", headers: { "Authorization": `Bearer ${token}` } });
