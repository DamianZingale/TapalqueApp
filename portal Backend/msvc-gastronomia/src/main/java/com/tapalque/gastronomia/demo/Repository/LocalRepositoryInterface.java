...
public interface LocalRepositoryInterface extends JpaRepository<Restaurant, Long> {
    ...
    // Seleccionar todos los restaurantes con categorías, teléfonos, horarios e imagen
    @Query(value = """
        SELECT
        ...
        GROUP BY r.id_restaurant, r.name, r.address, r.email,
                 r.latitude, r.longitude, r.delivery, r.delivery_price
        ORDER BY r.id_restaurant
        """, nativeQuery = true)
    List<RestaurantDTO> selectAllRestaurant();


    // Seleccionar restaurante por ID con categorías, teléfonos, horarios e imagen
    @Query(value = """
        SELECT
            r.id_restaurant AS idRestaurant,
            r.name,
